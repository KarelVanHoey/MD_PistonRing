...
t0 = (np.abs(Time.t - InitTime)).argmin()
...
####################################################################################
## Test of Load Balance at a given t, WITHOUT squeeze term or temperature effects ##
####################################################################################
...
for time in [120]: #[120, 450, 2000, 3000, 4500] # Calculation for t = time/100 ms: see plot of U and F vs time; Note: choose t in such a way that U>>0!
    ...
    print("Maximum pressure " +str(max(StateVector[time].Pressure)/10**(-6))+ "MPa")
